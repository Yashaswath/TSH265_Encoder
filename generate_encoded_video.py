...
def run_ffmpeg_kvazaar(input_file, input_res, input_fps, low_qp, high_qp, tile_size):
    # Step 1: Format the generated mask values for kvazaar
    ...
    if os.path.exists(mask_file_name):
        ...
        print("Missing mask values in the given folder of videos: " + input_file)
        return
    ...
    # Step 3: Encode YUV to HEVC with Kvazaar using the formatted mask values
    mask_file_formatted_name = mask_file_name.replace(".txt", "_formatted.txt")
    kvazaar_command = [
        'kvazaar', '-i', yuv_output, '--input-res', input_res, '--input-fps', input_fps,
        '-q', str(low_qp), '--roi', mask_file_formatted_name, '-o', roi_output
    ]

    try:
        result = subprocess.run(kvazaar_command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print("Kvazaar command output:", result.stdout.decode())
        print("Kvazaar command error (if any):", result.stderr.decode())
    except subprocess.CalledProcessError as e:
        print("Kvazaar command failed with error:", e.stderr.decode())
        raise
